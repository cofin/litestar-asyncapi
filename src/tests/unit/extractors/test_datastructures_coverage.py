...
def test_schema_registry_reference_collision() -> None:
    ...
    field1 = replace(FieldDefinition.from_annotation(MyType), kwarg_definition=KwargDefinition(schema_component_key="same"))
    field2 = replace(FieldDefinition.from_annotation(OtherType), kwarg_definition=KwargDefinition(schema_component_key="same"))
