...
from litestar_asyncapi.asyncapi import generator
...
def test_infer_content_type_returns_none_for_untyped_schema() -> None:
    assert generator._infer_content_type(Schema()) is None


def test_infer_content_type_detects_json_from_ref_and_object() -> None:
    assert generator._infer_content_type(Reference(ref="#/components/schemas/User")) == "application/json"
    payload = Schema(type=SchemaType.OBJECT, properties={"id": Schema(type=SchemaType.INTEGER)})
    assert generator._infer_content_type(payload) == "application/json"


def test_infer_content_type_detects_json_from_array() -> None:
    assert generator._infer_content_type(Schema(type=SchemaType.ARRAY)) == "application/json"
