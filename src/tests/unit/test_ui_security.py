...
def test_playground_renderer_escapes_title() -> None:
    ...
    schema = {"info": {"title": '"><script>alert(1)</script>', "version": "1.0.0"}, "channels": {}}
    # Mock request

    class MockRequest:
        pass

    html_bytes = plugin.render(cast("Any", MockRequest()), schema)
...
def test_playground_js_escaping() -> None:
    ...
    html_bytes = plugin.render(cast("Any", None), {"info": {"title": "test", "version": "1.0.0"}, "channels": {}})
    html_str = html_bytes.decode("utf-8")
