...
3. ``dependencies`` is a non-empty list that includes ``litestar-asyncapi``.
...
def validate_file(path: Path) -> list[str]:
    ...
    if not any(d.split("[")[0].strip() == "litestar-asyncapi" for d in deps if isinstance(d, str)):
        errors.append(f"{path}: `dependencies` must include `litestar-asyncapi`")
    return errors
